...
class RegistrationChanges(View):
    ...
    def post(self, request, device_token, website_push_id):
        logger.info(
            "Change in registration for website {pid}: token {token}".format(
        ...
        try:
            ...
            token = Token.objects.create(
                token=device_token, status=Token.STATUS.granted, website_push_id=website_push_id
            )
            isnew = True
        else:
        ...
        permission_granted.send(
            sender=self.__class__, token=device_token, userinfo=userinfo, isnew=isnew
        )
        return HttpResponse("")

    def delete(self, request, device_token, website_push_id):
        logger.info(
            "Attempt to delete registration for website {pid}: token {token}".format(
                token=device_token, pid=website_push_id
            )
        )
        userinfo = self._get_userinfo(request)
        ...
        try:
            ...
            logger.error(
                "Token {token} was not registered for website {pid}".format(
            ...
            if token.status == Token.STATUS.denied:
                logger.info(
                    "Token {token} was already denied for website {pid}".format(
                        pid=website_push_id, token=device_token
                    )
                )
            else:
    ...
    def _get_userinfo(self, request):
        ...
        try:
            ...
            raise ValueError(
                "All APN registrations calls are expected to contain an authorization header"
            )
        apn, _, userinfo = authentication_header.partition(" ")

        if apn != "ApplePushNotifications":
            raise ValueError(
                "All APN authentication headers are expected to start with ApplePushNotifications"
            )

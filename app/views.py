...
def import_artists(request):
    ...
    if type == "last.fm":
        ...
        client = django_settings.LASTFM_CLIENT
        lastfm_user = client.get_user(username)
        artists = lastfm_user.get_top_artists(period=period, limit=count)

        for artist in artists:
            mbid = artist.item.get_mbid()
            if not mbid:
                continue
            if mbid in Artist.blacklisted:
                continue
            logger.info(f"Importing artist {mbid} for {request.user.username}")
            artist, created = Artist.objects.get_or_create(
                mbid=mbid, defaults={"name": artist[0].name, "sort_name": artist[0].name}
            )
            UserArtist.add(request.user, artist)
            if created:
                tasks.get_release_groups_by_artist.delay(artist_mbid=mbid)
